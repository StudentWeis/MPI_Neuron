...
def process_Neuron(niter, numNeuron):
    ...
    WeightMask = np.random.choice(
        [-1, 1, 0], size=(numNeuron, numNeuron * comm_size), p=[.2, .2, .6]) * (0.1)  # 兴奋型连接和抑制型连接
    ...
    for i in range(niter):
        ...
        ctl_lib.lifPI(VmR, Spike, numNeuron, Ij,
                      period)  # 使用 C 动态库进行大规模神经元电位计算
